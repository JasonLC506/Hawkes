...
def lamda_update(paras, lamda):
    ...
    for n in range(4):
        for i in range(len(lamda[n])):
            lamda[n][i] = paras[n+2]*i/paras[n+6]/paras[n+6]*math.exp(0-(i*i)/2/paras[n+6]/paras[n+6])
    for n in [4,5]:
        for i in range(lamda[n]):
            lamda[n][i] = paras[n-4]/pow(i,(paras[n-4]+1))
    return lamda
...
def paras_update(prob, series, paras):
    ...
    for n in [0,1]:
        paras_new[n] = sum(prob[n+4])/sum([math.log(i)*prob[n+4][i] for i in range(len(prob[n+4]))])
    for n in range(2,6):
        series_sum = [sum(series[:][i]) for i in [0,1]]
        paras_new[n] = sum([sum(prob[n-2][i])] for i in range(len(prob[n-2])))/series_sum[(n-2)/2]
    for n in range(6,10):
        paras_new[n] = sum([sum([prob[n-6][i][j]*math.pow(i-j,2) for j in range(i)])for i in range(len(prob[n-6]))])\
        /sum([sum(prob[n-6][i])] for i in range(len(prob[n-6])))
    return paras_new


def improve_cal(paras_old, paras_new):

    improve = []*len(paras_old)
